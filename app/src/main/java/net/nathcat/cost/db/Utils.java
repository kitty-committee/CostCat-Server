...
public class Utils {
  ...
  public static User[] getGroupMembers(Database db, int group) throws SQLException {
    ...
    try {
      q = db.newQuery(
          "SELECT SSO.Users.id as 'id', SSO.Users.username as 'username', SSO.Users.fullName as 'fullName', SSO.Users.pfpPath as 'pfpPath' FROM DataCat.`Groups` JOIN SSO.Users ON SSO.Users.id = `owner` WHERE DataCat.`Groups`.id = ?");
      q
          .set(1, Integer.class, group)
          .execute();
    } catch (NoSuchMethodException e) {
